...
const Services = () => {
  ...
  return <section id="services" className="py-20 bg-gray-900 text-white">
      <div className="container mx-auto px-4">
        {/* Custom Services Section */}
        <div className="grid lg:grid-cols-2 gap-12 mb-20">
          {customServices.map((service, index) => <div key={index} className="relative overflow-hidden rounded-lg">
              ...
              <div className="relative z-10 p-8 h-96 flex flex-col justify-center">
                ...
                {service.buttonAction === 'autoscout24' ? <Button onClick={openAutoScout24} className="bg-brand-orange hover:bg-orange-600 text-black font-bold py-3 px-6 w-fit animate-shake-button">
                    {service.buttonText}
                  </Button> : service.buttonAction === 'contact' ? <Button onClick={scrollToContact} className="bg-brand-orange hover:bg-orange-600 text-black font-bold py-3 px-6 w-fit">
                    {service.buttonText}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {autoHouseServices.map((service, index) => <Card key={index} className="hover:shadow-lg transition-shadow duration-300 bg-white border-gray-200">
